    ...
    public class FFXIVActionLayerHandler : LayerHandler<FFXIVActionLayerHandlerProperties>
    {
        ...
        public override EffectLayer Render(IGameState gamestate)
        {
            ...
            if (gamestate is GameState_FFXIV ffxiv && ffxiv.Actions.Any())
            {
                ...
                {
                    layer.Set(ffxivAction.Key, Utils.ColorUtils.MultiplyColorByScalar(Properties.PrimaryColor, ffxivAction.CoolDownPercent == 0 ? 100 : ffxivAction.CoolDownPercent / 100D));
                    if (ffxivAction.IsProcOrCombo)
                        layer.Set(ffxivAction.Key, Utils.ColorUtils.MultiplyColorByScalar(Properties.Combo, ffxivAction.CoolDownPercent == 0 ? 100 : ffxivAction.CoolDownPercent / 100D));
                    if (!ffxivAction.InRange)
                        layer.Set(ffxivAction.Key, Utils.ColorUtils.MultiplyColorByScalar(Properties.OutOfRange, ffxivAction.CoolDownPercent == 0 ? 100 : ffxivAction.CoolDownPercent / 100D));
        ...
        private void setModifKeys(EffectLayer layer, FFXIVAction[] actions, DeviceKeys[] deviceKeys)
        {
            ...
            if (actions.Any()) layer.Set(deviceKeys, Utils.ColorUtils.MultiplyColorByScalar(Properties.PrimaryColor, scalar));
